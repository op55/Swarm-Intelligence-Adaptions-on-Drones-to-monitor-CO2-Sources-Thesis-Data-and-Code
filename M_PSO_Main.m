function[] = M_PSO_Main(parameters, environment, drones, locations, run)
    ...
    % PSO Velocity Values:
    weight        = 0.7;        % The weight of movement.
    cognitive     = 0.5;        % Inertia rate.
    social        = 1.5;          % Correlation rate.
    ...
    % Global best entries:
    gBestPos = [0, 0, 0];   % Global best position.
    gBestVal = 0;           % Global best value.
    ...
    for iter = 1:iterations
        ...
        for d = 1:numDrones
            ...
            if (status == "TAKEOFF")
                ...
                % Commence M-PSO role once desired altitude is reached:
                if (drones(d).POSIT(Z) == minAltitude)

                    % Used to determine the drone's M-PSO role:
                    gatherCheck = false;

                    % Check if any drones are trying to gather to a place:
                    for g = 1:numDrones
                        if (drones(g).STATUS == "M-PSO" && drones(g).M_ROLE == "GATHER")
            ...
            %% The M-PSO algorithm's actions:

            % The drone is currently trying to do M-PSO:
            if (status == "M-PSO")
                % Getting the drone's current M-PSO action:
                role = drones(d).M_ROLE;

                % The drone wants to use the M-PSO's velocity calculation:
                if (role == "GATHER")
                    % Shortening some drone-related variables:
                    dVal = drones(d).VALUE;
                    dPos = drones(d).POSIT;
                    dBestPos = drones(d).BEST_POSIT;
                    dVelocities = drones(d).VELOCITIES;

                    % Checking if the position is not the best:
                    if (dVal < gBestVal)
                        % Calculating a velocity for the drone:
                        drones(d).VELOCITIES = Get_Velocity(...
                            dimensions, weight, cognitive, social,... 
                            dVelocities, dVal, dPos, dBestPos,... 
                            gBestPos, gBestVal, upMove, loMove...
                        );

                        % Move drone if it lacks the global best position:
                ...
                if (role == "APPROACH")
                    ...
                    % Performing the same flag check as the M-PSO action:
                    if (drones(d).VALUE > minPollution)
                        if (polStates(nX, nY, nZ) > -1)
                ...
                if (role == "ESCAPE")
                    ...
                    if (drones(d).VALUE > minPollution)
                        ...
                        % Setting the drone to to do normal PSO movements:
                    else
                        drones(d).M_ROLE = "GATHER";
                    end
                end
        ...
        if (dronesArrived == (numDrones - dronesInBase))
            ...
            for d = 1:numDrones
                ...
                if (drones(d).ACTIVE == true)
                    ...
                    % Set PSO role if flags were found during approaches:
                    if (flagMade == true)
                    ...
                    % Setting each drone back to the 'M-PSO' status:
                    drones(d).STATUS = "M-PSO";
        ...
        if (mod(iter, stateCheck) == 0)
            ...
            % Resets each of the drones' states if in M-PSO:
            for d = 1:numDrones
            ...
            % Resetting the global bests:
            gBestVal = 0;
            gBestPos = zeros(1, dimensions);
        ...
        %% Checking for personal or global best values:

        for d = 1:numDrones
            % Check if the drone has a new personal best:
            if (drones(d).VALUE > drones(d).BEST_VALUE)
                drones(d).BEST_VALUE = drones(d).VALUE;
                drones(d).BEST_POSIT = drones(d).POSIT;
            end

            % Checks if the drone has got a new global best:
            if (drones(d).VALUE > gBestVal)
                gBestVal = drones(d).VALUE;
                gBestPos = drones(d).POSIT;
            end
        end
    ...
    Gather_Results("M-PSO", run, iter, timer, dimensions, areas, originPos,... 
